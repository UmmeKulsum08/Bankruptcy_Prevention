        ...
        if not required_columns.issubset(data.columns):
           st.error("This is not the Bankruptcy dataset! Please upload the correct file.")
            ...
            if model_choice == "Logistic Regression":
                 ...
                 n_estimators = st.slider("Select number of trees (for Random Forest)", min_value=10, max_value=200, value=100)
                 max_depth = st.slider("Select max depth (for Random Forest)", min_value=1, max_value=20, value=10)
                 model = RandomForestClassifier(n_estimators=n_estimators, max_depth=max_depth, random_state=42)
            elif model_choice == "SVM":
                 kernel = st.selectbox("Select kernel (for SVM)", ("linear", "rbf", "poly", "sigmoid"))
                 C = st.slider("Select C value (for SVM)", min_value=0.1, max_value=10.0, value=1.0)
